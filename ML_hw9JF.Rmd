...
covar = covariates %>%
  ...
  missmap(covar, main = "Missing values vs observed") %>% #No NA's
  print(max(covar$h_mbmi_None, na.rm = TRUE) - min(covar$h_mbmi_None, na.rm = TRUE)) %>% #range of h_mbmi_None
  print(max(covar$hs_wgtgain_None, na.rm = TRUE) - min(covar$hs_wgtgain_None, na.rm = TRUE)) %>% #range of hs_wgtgain_None
summary(covar$h_mbmi_None) %>% summary(hs_wgtgain_None) %>%
count(covar,h_edumc_None) %>% count(covar,h_native_None)
```
